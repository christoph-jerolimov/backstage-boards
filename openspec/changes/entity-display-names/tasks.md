...
- [ ] 1.1 Add `entityDisplayName(ref: string, entity?: Entity): string` to
...
- [ ] 1.2 Cover it in `refs.test.ts`: a user resolved by profile display
...
- [ ] 2.1 Rewrite the label branch of `useProfiles` to call
...
- [ ] 2.2 Add an `AssigneeAvatars.test.tsx` case for a `group:` ref whose
...
- [ ] 3.1 In `ItemMenu.tsx`, resolve the `assigneePool` through
...
- [ ] 3.2 Verify in the same test file that the menu renders its entries
      unchanged when the catalog resolves nothing (bare ref names, no
      crash), covering the fallback scenario

## 4. The ref as a tooltip

- [ ] 4.1 In `AssigneeAvatars.tsx`, give the single-assignee name a
      `TooltipTrigger`/`Tooltip` carrying the full ref, and extend the
      stacked-avatar tooltip to show the display name and the ref on
      separate lines; leave `text:` badges untouched; verify by test that
      hovering shows the ref and that a `text:` badge offers no tooltip
- [ ] 4.2 In `BoardFilterBar.tsx` and `ItemMenu.tsx`, attach the full ref
...
- [ ] 4.3 Verify the tooltip does not leak into the accessible name of
...
- [ ] 5.1 Run `yarn workspace @internal/plugin-boards test`,
      ...
      pass
- [ ] 5.2 Start the app (`yarn start`), assign a user with a profile
