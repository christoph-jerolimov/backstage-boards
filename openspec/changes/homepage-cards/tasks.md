...
- [x] 1.1 Add `BoardStatusCount` to `boards-common/src/types.ts` and the
...
- [x] 1.2 Extend `BoardsService.listBoards` with `withCounts`: after the
...
- [x] 1.3 Read `?counts=true` in the `GET /boards` route and pass it
...
- [x] 2.1 Add `withCounts` to `BoardsApi.listBoards` / `BoardsClient`
...
- [x] 2.2 Add `useBoardListQuery({ favoritesOnly, withCounts })` to
...
- [x] 2.3 Add `filterDueEntries(entries, now)` and
...
- [x] 3.1 Add `components/AssignedItemsWidget.tsx` — `Content` reading
...
- [x] 3.2 Render the grouped list — group label with count (board labels
...
- [x] 4.1 Add `components/BoardsWidget.tsx` — `Content` reading
...
- [x] 4.2 Render board rows — name linking to `<base>/<boardId>`, and
...
- [x] 5.1 Add `@backstage/plugin-home-react` to `plugins/boards/package.json`
      ...
      settings schema.
- [x] 5.2 Add both widgets to `app-config.yaml`'s `page:home`
...
- [x] 6.2 Add a Playwright case under `plugins/boards/e2e-tests` that
