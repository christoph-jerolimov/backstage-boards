...
- [x] 1.1 Move the `<table>` markup, `ToggleBadge`, `toggled`, the two
...
- [x] 1.2 Reduce `PriorityMatrixDialog` to its priority row derivation
      plus the dialog shell around `MatrixTable`, keeping its dialog
      title, caption, width, and aria labels byte-identical
- [x] 1.3 Verify the refactor is behavior-preserving: run
...
- [x] 2.1 Add `AssigneeMatrixDialog.tsx` deriving its rows from
      ...
      that group exists
- [x] 2.2 Label the rows with `useProfiles(refs)`'
      ...
      catalog lookup
- [x] 2.3 Render the dialog shell — title "Assignee matrix", the
...
- [x] 2.4 Verify `yarn workspace @internal/plugin-boards tsc` and
      `yarn workspace @internal/plugin-boards lint` pass

## 3. Wiring it into the board page

- [x] 3.1 Rename the `'matrix'` kind in `BoardDialogKind` to
...
- [x] 3.2 Add an "Assignee matrix…" `MenuItem` to the board actions menu
...
- [x] 4.1 Add `AssigneeMatrixDialog.test.tsx` asserting the grid for two
      ...
      appear in a cell
- [x] 4.2 Test that one item assigned to two people shows 1 in both rows
      and an overall total of 2, and that the double-count note is shown
- [x] 4.3 Test the "Unassigned" row: present with its counts and
      participating in the sums when unassigned items exist, absent when
      every item has an assignee, and last among the rows
- [x] 4.4 Test that clicking a status header excludes that column — the
...
- [x] 4.5 Test that reopening the dialog restores the fully selected
      state
- [x] 4.6 Add a `BoardPage.test.tsx` case opening the assignee matrix
