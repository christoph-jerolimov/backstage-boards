...
- [x] 1.1 Add `BoardListFilter` (`search?`, `entityRef?`, `createdBy?`,
...
- [x] 2.1 Add a private `visibleBoards(principal)` query builder to
...
- [x] 2.2 Add the parity test: for the matrix of five visibilities ×
      ...
      2.3.
- [x] 2.3 Rebuild `listBoards` on `visibleBoards`: drop the per-board
...
- [x] 3.1 Add `search` (case-insensitive `lower(name) like ?` with `%`,
      ...
      combining.
- [x] 3.2 Order by `name, id` and add `limit`/`offset` paging plus a
      ...
      the unpaged shape.
- [x] 3.3 Attach `withCounts` status counts to the returned page only
      (not the whole matching set). Verify counts still appear for a
      paged listing and cost one query.
- [x] 3.4 Add `listFilterOptions(principal)` returning
...
- [x] 4.1 Read `search`, `createdBy`, `limit` and `offset` in
...
- [x] 4.2 Register `GET /boards/facets` **before** `/boards/:boardId`
...
- [x] 5.1 Change `BoardsApi.listBoards` / `BoardsClient` to take the
      ...
      result shape.
- [x] 5.2 In `queries.ts`, add `useBoardsPageQuery(params)` under
...
- [x] 5.3 Widen the invalidations that today use
...
- [x] 6.1 Add `TablePagination` (range summary, previous/next disabled
      ...
      page-size change.
- [x] 6.2 Add `BoardsFilterBar` in `ItemFilterBar`'s shape: 240px
...
- [x] 6.3 Wire `BoardListPage`: per-tab paged queries (Favorites with
...
- [x] 6.4 Move the page's loading/error/empty handling onto `AsyncList`
...
- [x] 7.1 `yarn tsc`, `yarn lint:all`, `yarn test`.
- [x] 7.2 Smoke the Boards page against the dev app with more boards
