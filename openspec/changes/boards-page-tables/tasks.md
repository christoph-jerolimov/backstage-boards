...
- [x] 1.1 Add `RowMenu.tsx` with `useRowContextMenu`, `RowContextMenu`,
      and `RowActionsMenu`; rebuild `ItemContextMenu` on it and verify
      the board table's row menu and right-click still open the same
      menu (`yarn tsc`, `yarn lint:all`)

## 2. Boards page

- [x] 2.1 Remove the "My items" header button from `BoardListPage` and
      verify the "My items" tab and the `/my-items` route still reach
      the listing
- [x] 2.2 Replace `BoardRows` with a `BoardsTable` (Favorite, Name,
...
- [x] 2.3 Add `BoardMenu` (Open board, add/remove favorite) to the
...
- [x] 3.1 Render each board group in `MyItemsList` as a table (Item,
      Status, Due, Tags, Actions) and verify a row click opens the item
      on its board
- [x] 3.2 Add `MyItemMenu` (Open item, Open board) to the Actions cell
      and wire right-click; verify from both the tab and the standalone
      page

## 4. Verification

- [x] 4.1 `yarn tsc`, `yarn lint:all`, `yarn test`, and a smoke run of
