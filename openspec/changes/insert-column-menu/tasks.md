...
- [x] 1.1 Widen `BoardActions.addColumn` in `BoardView.tsx` to
      ...
      passes unchanged
- [x] 1.2 Assert in a `BoardPage.test.tsx` (or `BoardView.test.tsx`) case
...
- [x] 2.1 Replace `KanbanView`'s `addingColumn` boolean with
...
- [x] 2.2 Add `onInsertBefore` / `onInsertAfter` callbacks to
...
- [x] 2.3 Compute the created position with
...
- [x] 3.1 Test that "Insert column after" on "Todo" (board: "Todo",
...
- [x] 3.2 Test that "Insert column before" on the leftmost column
      produces a position below that column's, placing the new column
      first
- [x] 3.3 Test that cancelling the inline field (Escape) and confirming
      an empty title both leave `addColumn` uncalled

## 4. Verification

- [x] 4.1 Run `yarn workspace @internal/plugin-boards test`,
      `yarn prettier:check` and `yarn lint:all`; verify all pass
- [x] 4.2 Start the app (`yarn start`), open a board with columns, insert
