...
- [x] 1.1 Export a shared `BOARDS_ENTITY_LABEL = 'boards'` constant (and its `'true'` value) from `@internal/plugin-boards-common`; verify it is re-exported from the package index and consumed by the later tasks rather than string literals
...
- [x] 3.1 Set `filter: { 'metadata.labels.boards': 'true' }` on `entityBoardsContent`; extend `plugin.test.tsx` to assert the extension carries the filter and that it matches a labelled entity but not an unlabelled one
