...
- [x] 2.2 Build the group's `ItemActions` from `boardsApi` bound to the
      ...
      error slot
- [x] 2.3 Replace `MyItemMenu` with `ItemMenu` in both the row actions
      menu and the right-click menu, passing "Open board" as
      `extraItems`
- [x] 2.4 Status cell renders `StatusBadge` from the resolved column,
      falling back to the entry's `columnTitle` badge while the board
      loads

## 3. Verification

- [x] 3.1 Update `MyItemsPage.test.tsx`: menu shows Open details, Move
...
- [x] 3.2 `ItemMenu.test.tsx` covers `extraItems` rendering
- [x] 3.3 `yarn tsc`, unit tests, lint
- [x] 3.4 Playwright smoke on the boards page "My items" tab: move an
